...
- **工具能力约束（`CapabilityProvider`）**：实现者把关键约束追加到工具描述——agnes_video（限流 1req/min 须串行、时长上限 ~18s、32 对齐、**轮询以 video_id 为键**：video_id 端点是唯一取成品 URL 端点且更可靠，须传 video_id 轮询）、doubao（size 档位按模型：5.0 lite 为 2K/3K/4K、或 WxH 像素，两种都支持不可混用）、agnes_ai（size 归一化）。**供应商偏好只进各自 Capabilities，transport 通用 schema/描述保持中性**（P2 教训：抽象层不写供应商偏向，generateVideo 通用句也不承诺具体返回标识符）。`getVideoResult` schema 用 `anyOf` 约束 task_id/video_id 至少传一个。
...
- **视频轮询 not-found 处理（2026-08-07，诚实暴露）**：`isTaskNotFound` 判定确定性 not-found（404 任意 body、或 400 且 body 含 `task_not_exist`）；`probe()` 一轮内**所有**被查端点都 not-found → `pollTask` **立即返 `failed`** + `video task not found (task_id=..., video_id=...)`，不当瞬时错误重试到 cap 返 working（避免"任务已消失却永久 working"的假象）。瞬时错误（429/5xx/网络）行为不变。取舍：仅 task_id 单端点偶发 404 会误报，靠 Capabilities 引导 agent 传 video_id 双端点规避。
- **供应商协议路径硬编码（不配置化）**：供应商协议路径/字段（Agnes 的 `/agnesapi?video_id=`、`/v1/videos/<task_id>` 等）是协议语义而非部署配置，硬编码于各自 adapter；部署可变性由 `base_url` 承担。同构但路径/字段不同的供应商走 `HTTPGenericVideoAdapter` 可配字段（`task_id_field`/`status_field`/`video_url_field`），不给专属 adapter 加配置面（YAGNI）。
