...
- **工具描述能力约束（`CapabilityProvider`）**: 实现该可选接口的 adapter 会把自己的约束说明追加到工具描述。agnes_video 声明「创建限流 1 请求/分钟须串行、时长上限 ~18s（num_frames≤441@24fps，超长 clamp）、输出 32 对齐不保证精确尺寸、**轮询以 video_id 为键**（video_id 端点是唯一能取成品 URL 的端点、比 task_id 端点更可靠，轮询必须传 video_id 而非仅 task_id）」；doubao 声明「size 档位（按模型：5.0 lite 为 2K/3K/4K）或 WxH 像素两种方式都支持、不可混用」；agnes_ai 声明 size 归一化规则。**供应商偏好只出现在各自 Capabilities 里，transport 通用 schema/描述保持中性**（generateVideo 通用句不承诺具体返回标识符，仅指引轮询工具名）
...
- **视频轮询容错**: Agnes 状态接口会间歇性返回 404 / 429，但任务在服务端仍会跑完。**瞬时错误**（429/5xx/网络错误）在双端点间回退重试，仅受单次 `getVideoResult` 的 `statusPollCap` 约束；cap 耗尽仍非终态则返回 `working`（不报错、不放弃任务），由调用方再次轮询。**确定性 not-found**（一轮内所有被查端点都返回 400 `task_not_exist` 或 404 `task not found`）则立即返回 `failed` 并注明 task_id/video_id，不掩盖成 `working`——诚实暴露任务已不存在，不做自动重试
...
- **供应商协议路径硬编码（不配置化）**: 供应商协议路径/字段（如 Agnes 的 `/agnesapi?video_id=`、`/v1/videos/<task_id>` 状态端点）是协议语义而非部署配置，硬编码于各自 adapter，不做配置化——部署可变性已由 `base_url` 承担。同构但路径/字段不同的供应商走 `HTTPGenericVideoAdapter` 的可配字段（`task_id_field`/`status_field`/`video_url_field`），不给专属 adapter 加配置面
